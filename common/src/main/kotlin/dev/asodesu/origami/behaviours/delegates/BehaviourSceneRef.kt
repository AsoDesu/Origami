...
class BehaviourSceneRef<V : Behaviour>(private val clazz: KClass<V>) : ReadOnlyProperty<Behaviour, V> {
    private var ref: V? = null

    override fun getValue(thisRef: Behaviour, property: KProperty<*>): V {
       if (ref == null || ref?.internalScene == null)
    ...
    private fun init(thisRef: Behaviour) = thisRef.internalScene?.getOrNull(clazz).also { ref = it }
}